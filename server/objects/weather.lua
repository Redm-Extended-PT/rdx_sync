Sync.WeatherInitialize = function()
    ...
    self.syncWeather = function(source)
        if (source) then
            TriggerClientEvent('rdx_sync:updateWeather', source, self.currentWeatherType, self.getWindSpeed())
        else
            TriggerClientEvent('rdx_sync:updateWeather', -1, self.currentWeatherType, self.getWindSpeed())
        end
    end
