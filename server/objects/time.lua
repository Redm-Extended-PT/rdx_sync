Sync.TimeInitialize = function()
    ...
    self.currentTime = math.floor(GetGameTimer() / 1000)
    ...
    self.addSeconds = function(addSeconds)
        if (addSeconds >= 0 and addSeconds <= 59) then
            self.currentTime = math.floor(self.currentTime + addSeconds)
    ...
    self.syncTime = function(source)
        ...
        if (source) then
            TriggerClientEvent('rdx_sync:updateTime', source, hour, minute, seconds)
        else
            TriggerClientEvent('rdx_sync:updateTime', -1, hour, minute, seconds)
        end
    end
